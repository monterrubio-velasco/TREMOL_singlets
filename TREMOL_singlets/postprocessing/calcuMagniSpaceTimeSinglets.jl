...
# Se aislan todos los eventos avalancha (tiempo de ocurrencia, x, y)

    for i=1:length(VecNew[:,1]) 
        if VecNew[i,4] == 1
            contador += 1     
            VecNewAval[contador,1] = VecNew[i,1]  #numero acumulado de eventos
            VecNewAval[contador,2] = VecNew[i,2]  #tiempo acumulado
            VecNewAval[contador,3] = VecNew[i,3]  #tiempo interevento
            VecNewAval[contador,4] = VecNew[i,4]  #codigo normal o avalancha
            VecNewAval[contador,5] = VecNew[i,8]  #posición X
            VecNewAval[contador,6] = VecNew[i,9]  #posiciÓN y        
        end     
    end  
  
    rdist = 0.0
    rmin = sqrt(2) # Se agrupan las avalanchas tomando en cuenta solo a los Vecinos cercanos
...
#         if (VectorCenterAperities[1,1]-VectorSaOriLateralSize_y[1]/2) <= i <= (VectorCenterAperities[1,1]+ VectorSaOriLateralSize_y[1]/2) && (VectorCenterAperities[1,2]-VectorSaOriLateralSize_x[1]/2) <= j <= (VectorCenterAperities[1,2]+VectorSaOriLateralSize_x[1]/2) &&
        ...
        if (rdist <= rmin) || (VectorCenterAperities[1,1]-VectorSaOriLateralSize_y[1]/2) <= VecNewAval[j,5] <= (VectorCenterAperities[1,1]+ VectorSaOriLateralSize_y[1]/2) && (VectorCenterAperities[1,1]-VectorSaOriLateralSize_y[1]/2) <= VecNewAval[j-1,5] <= (VectorCenterAperities[1,1]+ VectorSaOriLateralSize_y[1]/2) || (VectorCenterAperities[1,2]-VectorSaOriLateralSize_x[1]/2) <= VecNewAval[j,6] <= (VectorCenterAperities[1,2]+ VectorSaOriLateralSize_x[1]/2) && (VectorCenterAperities[1,2]-VectorSaOriLateralSize_x[1]/2) <= VecNewAval[j-1,6] <= (VectorCenterAperities[1,2]+ VectorSaOriLateralSize_x[1]/2) 
            ...
            VecNewAvalSpaceTime[contNumAvalSpaceTime,4] = VecNewAval[j,3]  # tiempo interevento
    ...
    for i=1:contAva
        if VecAvalanchas[i] >= 2
            ...
            DuraciAval[r] = sqrt(VecArea[r])/velAsp #DurSingCell *sqrt(VecAvalanchas[i])    
            VecRateRuptAval[r] = VecAval[r] / VecTimeRuptureAvalanche[r]  #velocidad de ruptura adimensional      
            if VecAval[r] > maxAv
                maxAv = VecAval[r]
...
# relaciones tomadas de Q. Rodriguez and Ottemöller (2013) para asperezas	
...
# ==============..............Ramirez..............====================================
    ...
    vecMagni[4,6] = 0 #aRamiAsperSilva 
    vecMagni[4,7] = 0 # qRamiAsperSilva 
    vecMagni[4,8] = 0 #minRamiAsperSilva 
    vecMagni[4,9] = 0 # aRamiAsperTelesca
    vecMagni[4,10] = 0 #qRamiAsperTelesca 
    vecMagni[4,11] = 0 #minRamiAsperTelesca
    vecMagni[4,12] = 0  
...
# ==================================================================================================
 ...
 # ================= Ruoture Duration vs Number of simulated earthquake ================= 

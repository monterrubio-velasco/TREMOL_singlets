...
# *** STAGE 2: If the cell has a strength larger of 1, the stage two starts and we search other cell that has a strength equal to one and a larger load to fail.
        ...
        if VecAsperi[iout,jout] > 1  # If the straone or more cells overpass the threshold load we go to this if statment (Avalanche event)o avalancha
            ...
            for i = 1:length(VectorAsperiArray)           
                if  VectorAsperiArray[i] == 1 && VectorPArraycopy[i] > 0.0  # search among all the elements of the matrix that have stregth = 1 and that is not the element that has just been chosen
                    contFuera += 1
